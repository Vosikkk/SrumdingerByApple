...
actor SpeechRecognizer: ObservableObject {
    ...
    init() {
        ...
        guard recognizer != nil else {
            transcribe(RecognizerError.nilRecognizer); return
        }
    ...
    nonisolated private func transcribe(_ error: Error) {
        ...
        if let error = error as? RecognizerError {
            errorMeassge += error.message
        } else {
            errorMeassge += error.localizedDescription
        }
        Task { @MainActor [errorMeassge] in
            transcript = "<< \(errorMeassge) >>"
        }
    }
}
